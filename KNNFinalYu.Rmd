...
The base-rate for the data set is 26.47%. 
```{r, echo = FALSE, include = FALSE}
...
# Choosing the best K value for kNN
  Using the elbow chart, we determined that a K value of 9 was best for our analysis. 
```{r, echo = FALSE}
...
The confusion matrix below illustrates some of the the evaluation metrics for the model.  The accuracy of the model overall is 82.35% which is good considering that the base rate for the data set was 26.47%. The Balanced Accuracy for the model was around 74.06% which is also pretty good compared to the base rate.  The false negative rate was 44.74% which is not great since that means 44.74% of people who had a stroke were classified as not being at risk of a stroke from our model.  The true negative rate was 92.86% which is great since that means the model was good at figuring out people who aren't at risk of a stroke. The Kappa value for the model was 0.5234 which indicates moderate agreement. For a model like this, it would have been better if the false negative rate was much lower and the false positive rate to be higher since it would be very bad if a patient who was at risk was told that they didn't have to worry about anything. The Log-Loss was calculated to be 0.8960143 which is not great.  The F1-score was 0.2739726 which is bad likely due to the high rate of false negatives.  
```{r, echo = FALSE, include = FALSE}
...
F1_Score(as.numeric(stroke_KNN), as.numeric(test_stroke$stroke)) # 0.2739726

```

# ROC/AUC Output
The AUC curve had a value of 0.8258593 which is pretty good meaning that our model is doing a good job of distinguishing between patients with and without disease.
```{r, echo = FALSE, include = FALSE}
...
print(stroke_perf_AUC@y.values) # AUC 0.8258593
